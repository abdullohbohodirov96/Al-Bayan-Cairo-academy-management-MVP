export function AddStudentForm({ onSubmit, branches = [], prefill }) {
  return (
    <form id="add-student-form" onSubmit={onSubmit}>
      <div className="grid2">
        ...
        <label className="field">Уровень
          <select name="level" defaultValue={prefill?.level || 'A1'}>
            <option>A1</option><option>A2</option><option>B1</option><option>B2</option><option>C1</option><option>C2</option>
          </select>
        </label>
        <label className="field">Группа
          <input name="group" required defaultValue={prefill?.group} placeholder="A1 — Weekend" readOnly={Boolean(prefill?.group)} />
        </label>
        <label className="field">Преподаватель
          <input name="teacher" required defaultValue={prefill?.teacher} placeholder="Ustoz Ahmad" readOnly={Boolean(prefill?.teacher)} />
        </label>
        ...
        <label className="field">Филиал
          <select name="branch" defaultValue={prefill?.branch || branches[0]?.name}>
